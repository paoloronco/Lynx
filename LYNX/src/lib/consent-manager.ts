...
_iub.csConfiguration = {
  ...
  private _observeDataLayerConsentSignals(): void {
    const win = window as any;
    if (!Array.isArray(win.dataLayer) || win.__lynxDataLayerConsentObserverInstalled) return;
    win.__lynxDataLayerConsentObserverInstalled = true;

    const dataLayer = win.dataLayer;
    const originalPush = dataLayer.push;
    if (typeof originalPush !== 'function' || originalPush.__lynxConsentPushObserved) return;
    ...
    (wrappedPush as any).__lynxConsentPushObserved = true;
