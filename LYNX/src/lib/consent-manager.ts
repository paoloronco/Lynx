...
_iub.csConfiguration = {
  ...
  private _wireIubendaConsentEvents(): void {
    ...
    const poll = setInterval(() => {
      ...
      if (purposes || attempts >= 20) {
        clearInterval(poll);
        if (purposes) syncIubendaConsent(false);
      }
    }, 250);
  }
   ...
   * This sync does not auto-dispatch scripts until explicit user interaction is
   * observed because it may be generated by implicit defaults.
   */
