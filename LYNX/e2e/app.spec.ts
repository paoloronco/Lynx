...
test.describe('Lynx Application Flow', () => {
  test('should complete first-time setup, edit profile, add a link, and verify public page', async ({ page }) => {
    ...
    await expect(page.getByText('Welcome to Lynx')).toBeVisible();
    ...
    // 2. Verifica reindirizzamento al pannello Admin e modifica del Profilo
    // Dovrebbe apparire l'header dell'admin
    await expect(page.locator('h1.admin-title, .admin-title')).toHaveText('Lynx Admin');
    
    // Selezioniamo esplicitamente la scheda "Profile" per gestire stati di inizializzazione transitori
    const profileTabTrigger = page.getByRole('tab', { name: 'Profile' });
    ...
    const nameInput = page.getByPlaceholder('Your name', { exact: true });
    await expect(nameInput).toBeVisible();
    await nameInput.clear();
    await nameInput.fill('Mario Rossi');

    const bioInput = page.getByPlaceholder('Tell people about yourself...');
