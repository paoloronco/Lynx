...
The repo root contains Docker/CI configuration. All application code lives under `LYNX/`:
...
All frontend commands must be run from `LYNX/`; all backend commands from `LYNX/server/`.

## Development commands

### Frontend (run from `LYNX/`)
```bash
npm install          # install frontend deps
npm run dev          # Vite dev server at http://localhost:5173
npm run build        # production build into LYNX/dist/
npm run lint         # ESLint
```

### Backend (run from `LYNX/server/`)
```bash
...
### Full production start (from `LYNX/`)
```bash
...
### Docker
```bash
# Build from repo root (Dockerfile is in LYNX/)
docker build -t lynx ./LYNX
docker run -p 3001:3001 -e JWT_SECRET=<secret> lynx
```
...
### Frontend (`LYNX/src/`)
- **`pages/Index.tsx`** — public profile page (read-only, no auth required)
...
### Database schema (SQLite, `LYNX/server/lynx.db`)
...
- In **production/Docker**: backend on `:3001` serves `LYNX/dist/` as static files; frontend and API are same-origin (`/api/*`)
...
| `DATA_DIR` | `__dirname` | Where `lynx.db` and `uploads/` live (set to `/app/data` in Docker) |
